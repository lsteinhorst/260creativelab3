...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getUserComments(context,user) {
      return axios.get("/api/users/" + user.id + "/comments").then(response => {
	context.commit('setFeedView',response.data.comments);
      }).catch(err => {
	console.log("getUserComments failed:",err);
      });
    },
    ...
    getFeed(context) {
      ...
      doSearch(context,keywords) {
            axios.get("/api/comments/search?keywords=" + keywords).then(response => {
             context.commit('setFeed',response.data.comments);
            }).catch(err => {
